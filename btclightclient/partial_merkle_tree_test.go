package btclightclient



import (
	"bytes"
	"encoding/hex"
	"fmt"
	"testing"

	"github.com/btcsuite/btcd/chaincfg/chainhash"
	"gotest.tools/assert"
)

func TestVerifyTxOutProof(t *testing.T) {
	// data from chainstack example: https://docs.chainstack.com/reference/bitcoin-gettxoutproof
	hexStr := "";
	proof, _ := hex.DecodeString(hexStr[160:]);
	fmt.Println(len(hexStr));

	reader := bytes.NewReader(proof)

	pmk, err := readPartialMerkleTree(reader, proof)

	assert.NilError(t, err)

	height := pmk.Height();
	nUsedBit := uint32(0);
	nUsedHash := uint32(0);
	txID, _ := chainhash.NewHashFromStr("0bf82c1d62b73497de2d796636cb1ce64415d25982332436007c0f51b5a75a62");
	merkleProof, err := pmk.computeMerkleProofRecursive(height, 0, &nUsedBit, &nUsedHash, txID)

	assert.NilError(t, err)
	assert.Equal(t, merkleProof.nodeValue.String(), "e95de06cb47c1732bc650b2c928f0f84c7946dec5d4c30276105515be1ea6947");
}
